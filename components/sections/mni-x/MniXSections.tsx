          ...
          {products.map((p) => (
            <div key={p.tier} className="p-8 transition-all duration-500 hover:scale-[1.02]"
              ...
              <div className="mb-6 flex items-start justify-between gap-3">
                <span className="font-bold leading-none" style={{ fontFamily: DISPLAY, fontSize: "3rem", color: `${ORCHID}40` }}>{p.tier}</span>
                <span className="mt-1 border px-3 py-1 text-[9px] tracking-widest uppercase" style={{ borderColor: `${ORCHID}40`, color: ORCHID, fontFamily: BODY }}>
                  {p.badge}
                </span>
              </div>
                ...
                <ul className="mt-4 space-y-2">
                  {p.kits.map((kit) => (
                    <li key={kit} className="flex items-center gap-2 text-xs text-white/50" style={{ fontFamily: BODY }}>
                      <span style={{ color: ORCHID }}>/</span> {kit}
                    </li>
                  ))}
                </ul>
              )}
              {p.note && (
                <p className="mt-4 border-l-2 pl-3 text-xs italic text-white/40" style={{ borderColor: `${ORCHID}40`, fontFamily: BODY }}>
                  {p.note}
                </p>
              )}
            </div>
